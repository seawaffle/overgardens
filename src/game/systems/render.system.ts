    ...
    if (level) {
      ...
      for (let x = 0; x < this.screenWidth; x++) {
        for (let y = 0; y < this.screenHeight; y++) {
          ...
          this.game.render.draw(x, y, tile.character, color, null);
        }
      }
    }
  }
      ...
      for (const { position, renderable } of this.renderQuery) {
        if (
          ...
          if (renderable.glyph.fg) {
            fg = renderable.glyph.fg;
          }
          if (renderable.glyph.bg) {
            bg = renderable.glyph.bg;
          }
          this.game.render.draw(pos.x, pos.y, renderable.glyph.character, fg, bg);
        }
      }
    }
  }
